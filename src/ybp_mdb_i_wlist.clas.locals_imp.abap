...
CLASS lhc_WList IMPLEMENTATION.
  ...
  METHOD mark_as_watched.
    ...
    loop at watching into data(lines).
        data(status) = 'X'.
        IF lines-pegi > ycl_mdb_calculateage=>calc_age( lines-birthday ).
            status = ''.
  ...
  METHOD validateWatchStatus.
    ...
    into table @data(watching).
     ...
     loop at wlists_result into data(wlist).
        loop at watching into data(lines).
            IF lines-wlistuuid = wlist-WListUuid and wlist-WatchStatus = 'X' and lines-pegi > ycl_mdb_calculateage=>calc_age( lines-birthday ).
